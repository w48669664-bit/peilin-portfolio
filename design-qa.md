...
## iOS/touch scrolling and sustained water audio — 2026-09-25

- Root cause: mobile `.home-ui` had `pointer-events:none`, so blank-area swipes hit a fixed water sibling with `touch-action:none`. The scroll container now receives its whole surface, with native touch/scroll defaults. Touch gestures and desktop pointer gestures are separate; a >8px initial move relinquishes the gesture to scrolling, while a stationary 320ms hold deliberately enters water drawing. Multi-touch, native scrolling/cancellation, blur, panel opening and teardown cancel drawing.
- Water audio retains the CC0 recordings. A sustained crossfaded stream follows slow movement and quiet stationary holds, ending with a 120ms fade. No hover-started audio, no delayed playback after release, no background sound scheduling. Safari's initial completed-tap requirement is documented; `touchend` supplies the audio activation fallback. No physical iPhone speaker/headphone listening is claimed.
- Added viewport-fit=cover, safe-area padding for scrolling/reading/lightbox content, iOS text-size control, native overflow scrolling, and coarse-pointer target sizing. Mobile resume navigation now starts below the close button (measured nav top60px vs close bottom57px at320px). Screenshot swipe/click are mutually exclusive; multi-touch and cancellation reset swipe state.
- Time-wheel layout is fluid at intermediate landscape sizes, can scroll vertically if needed, binds to one primary pointer, clips the rotating square inside its circular boundary, and gives the native time input enough width for all digits.
